...
//    }
  ...
  func predictUsingVision(ciImage: CIImage) {
    ...
    try? handler.perform([request]) //VNCoreMLRequestを実行
  }
...
//            var flgAllComp = false
//            if self.detects.hasCoffee() && self.detects.hasItem01() && self.detects.hasItem02() {
////                print("all items complete!")
//                flgAllComp = true
//                self.hideAICMsgCount = 0
//            } else {
//                if self.hideAICMsgCount < 30 {
//                    self.hideAICMsgCount += 1
//                    flgAllComp = true
//                }
//            }
//            if flgAllComp {
//                let compMsg = "*** ALL Items is COMPLETED!! ***"
//                self.timeLabel.text = String(format: "%@ \nElapsed %.5f seconds - %.2f FPS", compMsg, elapsed, fps)
//            } else {
...
//        self.predictUsingVision(pixelBuffer: pixelBuffer)
        ...
        if self.isNeedRotate {
            // pixelBuffer を90度回転させる処理
            ciImage = ciImage.rotate
        }
        self.predictUsingVision(ciImage: ciImage)
      }
    }
  }
}
